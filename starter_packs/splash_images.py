...
def get_splash_background(width, height, attribution, font, cache_path=None):
    """
    ...
    for successive images of the same dimensions. Set `cache_path` to `None`
    to disable caching.
    """
...
def fetch_avatar(url, crop_mask):
    ...
    If it contains transparency, a gray background is applied before `crop_mask`.
...
def is_character_in_font(character, font_path):
    if font_path not in TTFONT_CACHE:
        TTFONT_CACHE[font_path] = TTFont(font_path, 0, allowVID=0, ignoreDecompileErrors=True, fontNumber=-1)
    for table in TTFONT_CACHE[font_path]["cmap"].tables:
